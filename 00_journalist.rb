...
# Quelle est le handle le plus court de cette liste ? array.length

puts "Quelle est le handle le plus court de cette liste ?"
print ">"
puts array.min_by(&:length)
...
if user == choice 4
    ...
    handle.each do |i|
        chars = i[1]
        if /[[:upper:]]/.match(chars) #méthode pour vérifier première lettre Majuscule
